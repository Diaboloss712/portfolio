...
MLflow로 실험을 체계적으로 관리하고, DVC + AWS S3로 대용량 생체 신호 데이터를 버전 관리하였습니다.`,
    ...
    details: [
      "PhysioNet EDF 신호 처리: 25Hz→100Hz 리샘플링, R-peak 검출, 30초 세그먼트로 55개 HRV 피처(시간·주파수·비선형) 자동 추출 파이프라인 구축",
      "이상치 제거(IQR, Z-score), 보간(선형·스플라인), 정규화(StandardScaler, MinMaxScaler) 등 다양한 전처리 조합 실험 및 최적화",
      "CatBoost·XGBoost·RandomForest 기반 정형 데이터 분류 모델 학습, GridSearchCV로 하이퍼파라미터 튜닝",
      "CatBoost(1차 분류) + Mamba/LSTM(시퀀스 보정) 2-stage 파이프라인으로 전이 구간 오분류 보정, F1-score 11.9% 향상",
      "GroupKFold 교차 검증으로 피험자 단위 데이터 분할, 데이터 누수 방지 및 일반화 성능 확보",
      "MLflow로 200+ 실험 추적(하이퍼파라미터·메트릭·Confusion Matrix), DVC+AWS S3로 데이터 버전 관리",
      "상용 웨어러블 데이터(삼성 헬스 모니터) 검증: CatBoost+Mamba 모델이 실제 수면 데이터 대비 43.1% 일치율 달성"
    ],
    pdfDetails: [
      "PhysioNet EDF 신호를 리샘플링하고 HRV 피처 55개 자동 추출",
      "CatBoost+Mamba/LSTM 2-stage 파이프라인으로 F1-score 11.9% 향상",
      "MLflow 200+ 실험 관리, DVC+S3 데이터 버전 관리로 재현성 확보"
    ],
    issues: [
      {
...
기기의 상태는 MQTT로 조회 및 로그로 관리하며, 제조 완료 시 WebSocket을 통해 클라이언트에 즉시 알림을 전송하였습니다.`,
    ...
    details: [
      "PerfumeProtocol 설계: Top/Middle/Base 노트 구성, ml 단위 분사량, 디바이스 ID를 JSON으로 직렬화하여 MQTT 메시지로 전송하는 프로토콜 정의",
      "MQTT 메시지 인코딩: JSON 직렬화 후 Base64 인코딩하여 네트워크 전송 효율성 확보, 임베디드 기기에서 디코딩 후 제조 실행",
      "비율 계산 알고리즘: 사용자가 선택한 향수 레시피를 Top/Middle/Base 노트별 비율로 변환, 총량 대비 각 노트의 ml 단위 분사량 계산",
      "레시피 매핑 서비스: DB에 저장된 향수 레시피(노트 조합)를 PerfumeProtocol 포맷으로 변환하여 MQTT 발행",
      "CompletableFuture 비동기 응답 대기: MQTT 제어 명령 전송 후 임베디드 기기의 완료 응답을 비동기로 대기, 타임아웃(30초) 설정으로 무한 대기 방지",
      "중복 신호 필터링: 임베디드 기기의 중복 완료 신호를 방어하기 위해, 첫 처리 시 lastJobIdByDevice.remove()로 작업 컨텍스트를 소거하여 이후 중복 신호가 들어와도 필터링되도록 구현",
      "WebSocket 실시간 알림: 제조 완료 이벤트를 WebSocket으로 프론트엔드에 브로드캐스트, 사용자에게 즉시 알림 제공",
      "대시보드 모니터링: 제조 이력, 디바이스 상태, 레시피 통계를 관리자 대시보드에서 실시간 조회 가능하도록 구현",
      "JUnit 단위 테스트: 비율 계산 로직, 레시피 매핑, MQTT 메시지 인코딩/디코딩 등 핵심 비즈니스 로직에 대한 테스트 작성 및 검증"
    ],
    pdfDetails: [
      "PerfumeProtocol 설계 및 MQTT 메시지 인코딩으로 임베디드 제조기 제어",
      "중복 신호 필터링으로 DB 무결성 확보 및 트랜잭션 동기화 구현",
      "WebSocket 실시간 알림 및 대시보드 모니터링 시스템 구축"
    ],
    issues: [
      {
        title: "임베디드 기기의 중복 완료 신호 처리 문제",
        cause: "MQTT로 제조 명령을 전송하면 임베디드 기기가 제조를 완료한 후 완료 신호(status: 'completed')를 전송합니다. 그런데 네트워크 불안정이나 기기 측 재전송 로직으로 인해 동일한 완료 신호가 여러 번 수신되는 상황이 발생하였습니다. 이로 인해 DB에 동일한 제조 이력이 여러 번 저장되거나, 재고가 중복으로 차감되는 문제가 있었습니다.",
        solution: "작업 컨텍스트 소거(Consume) 방식으로 중복 신호를 필터링하였습니다. lastJobIdByDevice에 deviceId와 jobId 매핑을 저장하고, 첫 번째 완료 신호 처리 시 remove()로 매핑을 소거합니다. 이후 중복 신호가 들어오면 lastJobIdByDevice.get(deviceId)이 null을 반환하여 로직이 실행되지 않도록 필터링됩니다. 또한 요청 시점에도 ConcurrentHashMap.putIfAbsent()로 중복 요청을 차단하고, TransactionSynchronizationManager.afterCommit()으로 DB 커밋 후에만 MQTT 메시지를 발행하도록 동기화하였습니다.",
        result: "임베디드 기기의 중복 완료 신호가 완전히 필터링되어 DB 무결성이 확보되었고, 재고 중복 차감 문제가 해결되었습니다. 트랜잭션 동기화로 서버 DB와 디바이스 상태 간 불일치도 방지되었습니다.",
        comparison: "중복 신호 필터링 적용 전에는 네트워크 불안정 시 10회 테스트 중 3회 정도 중복 저장이 발생했으나, 적용 후 100회 이상 테스트에서 중복이 0회로 감소하였습니다."
      },
      {
...
크롤링한 문서를 섹션·컴포넌트 단위로 파싱하고, Pinecone 벡터 DB에 임베딩하여 유사도 기반 검색을 수행하였습니다.`,
    ...
    details: [
      "Bootstrap 문서 크롤링 및 LangChain RecursiveCharacterTextSplitter로 청크 단위 분할, OpenAI 임베딩 변환",
      "Pinecone 벡터 DB에 1536차원 임베딩 인덱싱, 네임스페이스로 Bootstrap 버전별 문서 분리 관리",
      "사용자 질의 임베딩 → Pinecone cosine 유사도 검색 → 상위 5개 문서 추출 → LLM 프롬프트 증강하는 RAG 파이프라인 구축",
      "LangChain ConversationBufferMemory로 Stateful 대화 관리, 이전 대화 맥락 유지하여 코드 생성 일관성 확보",
      "FastAPI 비동기 처리(async/await)로 벡터 검색과 LLM 호출 병렬화, 응답 속도 개선",
      "Pydantic 스키마로 요청/응답 데이터 검증, CORS 설정으로 프론트엔드와 안전하게 통신",
      "PyArrow read_table로 Pandas read_csv 대비 데이터 로딩 시간 84% 단축(2.5초→0.4초), 필요한 컬럼만 선택적 로딩하여 메모리 최적화",
      "Jenkins 파이프라인 구축: Git Push 시 자동으로 Docker 이미지 빌드 및 배포하는 CI/CD 워크플로우 구현",
      "Dockerfile 다단계 빌드와 환경변수 스코프를 stage 내부로 최소화하여 보안 강화, Credential 플러그인으로 민감 정보 관리"
    ],
    pdfDetails: [
      "Bootstrap 문서 크롤링 후 Pinecone 임베딩하여 RAG 파이프라인 구축",
      "PyArrow 마이그레이션으로 I/O 성능 84% 개선 및 Jenkins 빌드 성공",
...
GitHub API를 연동하여 일일 커밋 여부를 확인하고, 커밋이 없는 경우 자동으로 배치 커밋을 실행하는 기능을 구현하였습니다.`,
    ...
    details: [
      "FastAPI 기반 MCP(Model Context Protocol) Git 툴 서버 구축: diff, commit, push, 브랜치 목록, 현재 브랜치 조회 등 Git 명령 래핑",
      "Ollama Mistral 모델로 git diff 결과 분석하여 Conventional Commit 메시지 자동 생성(feat, fix, docs 등 타입 자동 분류)",
      "정규표현식으로 Conventional Commit 형식 검증(type(scope): description), 형식 오류 시 재생성 로직 구현",
      "LLM 생성 메시지에서 마크다운 코드 블럭(```), 따옴표, 불필요한 줄바꿈 제거하여 순수 커밋 메시지만 추출",
      "Windsurf IDE와 통합하여 개발자가 AI에게 커밋 요청 시 자동으로 diff 분석 → 메시지 생성 → 검증 → commit → push 워크플로우 실행",
      "에러 핸들링: Git 명령 실패 시 상세 에러 메시지 반환, LLM 생성 실패 시 재시도 로직 구현"
    ],
    pdfDetails: [
      "FastAPI-MCP 기반 Git 툴 서버로 diff, commit, push 명령 래핑",
      "Ollama Mistral로 Conventional Commit 메시지 자동 생성 및 형식 검증",
      "Windsurf IDE 통합으로 AI 기반 Git 워크플로우 자동화"
    ],
    issues: [
      {
        title: "FastAPI-MCP와 Ollama의 성능 병목 지점",
        cause: "FastAPI-MCP와 Ollama의 성능 병목 지점을 프로파일링한 결과, Git 명령어 실행과 HTTP 통신이 병목 지점으로 나타났습니다.",
        solution: "Git 명령어 실행을 subprocess로 대체하고, HTTP 통신을 httpx로 최적화하였습니다.",
        result: "성능 병목 지점을 최적화하여 Git 워크플로우 자동화 시스템의 성능을 향상시켰습니다.",
        comparison: "성능 병목 지점 최적화 전후 성능 비교 결과, 30%의 성능 향상을 달성하였습니다."
      }
    ],
    retrospect: [
...
Future 패턴을 활용한 비동기 status 응답 대기 메커니즘으로 MQTT 제어 명령 후 3초 이내 status를 받지 못하면 타임아웃 처리하였습니다.`,
    ...
    details: [
      "MQTT Broker 연동: paho-mqtt 라이브러리로 MQTT Broker와 연결, IoT 디바이스(Unity 시뮬레이터)에서 전송되는 감정 데이터를 실시간 수신",
      "WebSocket 실시간 통신: FastAPI WebSocket으로 클라이언트와 양방향 통신 채널 구축, 감정 분석 결과 및 음악 추천 정보 즉시 전달",
      "Capabilities 기반 동적 라우팅: 디바이스의 Capabilities(heart_rate, temperature 등)를 기반으로 동적 API 라우팅, 새로운 디바이스 타입 추가 시 JSON 설정만 변경하여 확장 가능",
      "MQTT 콜백과 FastAPI 이벤트 루프 분리: paho-mqtt 콜백이 별도 스레드에서 실행되어 FastAPI 메인 이벤트 루프와 충돌 방지, BackgroundTasks와 asyncio.Queue로 안전하게 비동기 작업 스케줄링",
      "Unity 시뮬레이터 연동: Unity에서 HTTP → MQTT → FastAPI → WebSocket → 웹 프론트 흐름으로 실시간 상태 동기화, 스마트홈 디바이스 제어 및 모니터링",
      "비동기 처리 최적화: FastAPI async/await로 MQTT 메시지 처리, 감정 분석, 음악 추천 로직을 비동기 처리하여 높은 처리량과 응답 속도 확보"
    ],
    pdfDetails: [
