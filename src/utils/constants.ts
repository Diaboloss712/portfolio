...
MLflow로 실험을 체계적으로 관리하고, DVC + AWS S3로 대용량 생체 신호 데이터를 버전 관리하였습니다.`,
    ...
    details: [
      "PhysioNet EDF 신호 처리 및 HRV 피처 자동 추출 파이프라인 구축",
      "이상치 제거, 보간, 정규화 등 다양한 전처리 조합 실험 및 최적화",
      "시계열 모델 Accuracy 9.6% 향상, Tumblr 모델 19.8% 향상",
      "CatBoost+Mamba/LSTM 앙상블 모델 및 GroupKFold 검증 전략 적용",
      "상용 웨어러블 데이터 검증(CatBoost+Mamba 삼성 헬스 대비 43.1% 일치)"
    ],
    issues: [
      {
...
기기의 상태는 MQTT로 조회 및 로그로 관리하며, 제조 완료 시 WebSocket을 통해 클라이언트에 즉시 알림을 전송하였습니다.`,
    ...
    details: [
      "PerfumeProtocol 설계 및 MQTT 메시지 인코딩을 통한 임베디드 제조기 제어 구현",
      "Top/Middle/Base 노트별 비율 계산 알고리즘 및 레시피 매핑 서비스 설계",
      "WebSocket 기반 제조 완료 실시간 알림 및 대시보드 모니터링",
      "JUnit 기반 단위 테스트 작성 및 핵심 비즈니스 로직 검증"
    ],
    issues: [
      {
...
크롤링한 문서를 섹션·컴포넌트 단위로 파싱하고, Pinecone 벡터 DB에 임베딩하여 유사도 기반 검색을 수행하였습니다.`,
    ...
    details: [
      "PyArrow 마이그레이션으로 Jenkins 빌드 실패 해결 및 I/O 성능 개선",
      "FastAPI RAG 백엔드 구현 및 Jenkins + Docker CI/CD 파이프라인 구축",
      "Jenkins 환경변수 최소화 및 Credential 분리로 보안 강화"
    ],
    issues: [
      {
...
GitHub API를 연동하여 일일 커밋 여부를 확인하고, 커밋이 없는 경우 자동으로 배치 커밋을 실행하는 기능을 구현하였습니다.`,
    ...
    details: [
      "FastAPI-MCP 기반 Git 툴 서버 구축(diff, commit, push, 검증 등)",
      "Ollama Mistral 기반 Conventional Commit 자동 생성 및 형식 검증",
      "마크다운 코드 블럭 및 따옴표 제거 후처리 로직 구현"
    ],
    issues: [
      {
...
Future 패턴을 활용한 비동기 status 응답 대기 메커니즘으로 MQTT 제어 명령 후 3초 이내 status를 받지 못하면 타임아웃 처리하였습니다.`,
    ...
    details: [
      ...
      "Capabilities 기반 동적 MQTT 라우팅 시스템 구현",
      "WebSocket 세션 관리 및 디바이스별 브로드캠스팅"
    ],
    issues: [
      {
